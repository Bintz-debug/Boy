...
module.exports.welcome = async(iswel, isleft, juna, anu) =>{
	try {
            ...
            for (let num of participants) {
  		      const fkontaku = { key: {participant: `0@s.whatsapp.net`, ...(anu.id ? { remoteJid: `6285774380068-1614953337@g.us` } : {}) }, message: { 'contactMessage': { 'displayName': ``, 'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;,;;;\nFN:,\nitem1.TEL;waid=${num.split('@')[0]}:${num.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, 'jpegThumbnail': setting.pathimg, thumbnail: setting.pathimg,sendEphemeral: true}}}
                try {
    ...
    var wc = await getBuffer(`https://telegra.ph/file/f1750a4e8d01a2230092e.png`)
                	await juna.sendMessage(anu.id, {image: wc, caption: `『 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 』
                	
Di grup chat, "${groupName}" !
Jangan rusuh dan jangan nakal ya, @${num.split("@")[0]}
Agar admin tidak marah

==============
BintangBotzMD\n`, mentions: [num]}, {quoted:fkontaku})
                }
   ...
   var lf = await getBuffer(`https://telegra.ph/file/a169e8779f511e1433e66.png`) 
await juna.sendMessage(anu.id, {image: lf, caption: `『 𝐒𝐀𝐘𝐎𝐍𝐀𝐑𝐀 』

Selamat tinggal, @${num.split("@")[0]}
Semoga kamu kembali lagi di grup ini, "${groupName}"
